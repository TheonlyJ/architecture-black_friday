# Запуск шардированного решения + репликация:
Все по прежнему должно работать так же.
```bash
docker compose up
```
...
Я потратил кучу времени и выдрал не буду говорить откуда кучу волос прежде чем понял что проблема в CRLF/LF. Убедитесь что строки оканчиваются LF в файле init-cluster.sh.

Опытным путём выяснилось что api_app не поддерживает кластеры Redis.
